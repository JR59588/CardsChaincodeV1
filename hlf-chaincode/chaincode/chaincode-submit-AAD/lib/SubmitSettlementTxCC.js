    ...
    try {
      ...
      for (let i = 0; i < x100Msgs.length; i++) {
        ...
        if (x100Verified) {
          ...
          console.log(`Submit Chaincode AAD: x100 message ${x100Msg.Key} is verified and status is changed to ${x100Msg.Record.TxStatus}`)
          await pymtutils.writeTxStatus(ctx, x100Msg.Key, channelName, x100Msg.Record);
        } else {
          x100Msg.Record.TxStatus = 'TxNotSubmitted';
          await pymtutils.writeTxStatus(ctx, x100Msg.Key, channelName, x100Msg.Record);
          console.log(`Submit Chaincode AAD: x100 message ${x100Msg.Key} is not verified and status is changed to ${x100Msg.Record.TxStatus}`)
        }
      ...
      if (!(x100TransactionAmountTotal == totalTransactionAmount)) {
        ...
        console.log("Submit Chaincode AAD: The settlement amount is not equal to total transaction amount of x100's");
      } else {
        x500Msg.TxStatus = 'TxSubmitted';
        let x500MsgUpdated = await pymtutils.writeTxStatus(ctx, key500, channelName, x500Msg);
        console.log("Submit Chaincode AAD: The settlement amount is equal to total transaction amount of x100's");
      }
      return x500Msg;
    } catch (error) {
